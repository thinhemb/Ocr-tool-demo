...
class DBPostProcess:
    ...
    def boxes_from_bitmap(self, pred, bitmap, shape, unclip_ratio, box_thresh=0.5):
        """
        ...
        contours = cv2.findContours(
            (bitmap * 255).astype(np.uint8), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]
    ...
    def __call__(self, outs, shape_list, unclip_ratio, box_thresh):
        return [
            self.boxes_from_bitmap(
                pred,
                cv2.dilate((pred > self.thresh).astype(
                    np.uint8), np.array([[1, 1], [1, 1]])),
